...
const viewNames = {
  ...
  sellers: 'Prodavači', finance: 'Financije', notes: 'Bilješke', changes: 'Povijest promjena', transactions: 'Povijest transakcija', settings: 'Postavke'
};
...
function renderDashboard() {
  ...
  const noteReminders=notes.filter((note) => note.status !== 'Završena' && note.status !== 'Arhivirana').sort((a,b) => Number(b.pinned)-Number(a.pinned)).slice(0,5).map((note) => `<div class="data-row"><div class="data-row-main"><strong>${note.pinned ? '◆ ' : ''}${escapeHTML(note.title)}</strong><small>${escapeHTML(note.priority)} · ${note.dueDate ? formatDate(note.dueDate,false) : 'Bez roka'}</small></div></div>`).join('');
  document.querySelector('[data-reminders]').innerHTML = noteReminders || emptyAdmin('Nema aktivnih podsjetnika.');
...
async function startAdmin() {
  ...
  bindNavigation();bindProducts();bindReservations();bindOrders();bindFinance();bindNotes();bindSellers();bindHistories();bindSettings();
