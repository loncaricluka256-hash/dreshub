...
function transactionRows(transactions) { return transactions.slice().sort((a,b)=>new Date(b.date)-new Date(a.date)).map(item=>{const profit=(item.items||[]).reduce((sum,sale)=>sum+(Number(sale.price)-Number(sale.costPrice))*Number(sale.quantity||1),0);return`<article class="transaction-row ${item.voided?'voided-transaction':''}" data-transaction-id="${escapeHTML(item.id)}"><div><span class="row-label">Datum</span><span class="row-text">${formatDate(item.date)}</span></div><div><span class="row-label">Opis</span><span class="row-text">${escapeHTML(item.description)}</span>${item.voided?'<span class="voided-badge">Poništeno</span>':''}</div><div><span class="row-label">Plaćanje / kanal</span><span class="row-text">${escapeHTML(item.paymentMethod||'-')} · ${escapeHTML(item.salesChannel||'-')}</span></div><div><span class="row-label">Izvor</span><span class="row-text">${escapeHTML(item.source||'Ručni unos')}</span></div><strong class="transaction-amount ${item.amount>=0&&!negativeTypes.has(item.type)?'positive':'negative'}">${formatPrice(item.amount)}</strong>${item.items?.length?`<div class="transaction-sale-items"><strong>Prodani dresovi · zarada ${formatPrice(profit)}</strong>${item.items.map((sale)=>`<a href="product.html?id=${escapeHTML(sale.productId)}" data-product-detail-link="${escapeHTML(sale.productId)}"><span>${escapeHTML(sale.name)} · ${escapeHTML(sale.player||'-')} · ${escapeHTML(sale.size||'-')}<small>Kupnja: ${formatPrice(sale.costPrice)} · Prodaja: ${formatPrice(sale.price)} · Zarada: ${formatPrice(sale.price-sale.costPrice)}</small></span><b>${formatPrice(sale.price)}</b></a>`).join('')}${!item.voided?`<button type="button" class="danger" data-void-sale="${escapeHTML(item.id)}">Poništi prodaju</button>`:''}</div>`:''}</article>`;}).join('')||emptyAdmin('Nema transakcija za odabrane filtre.'); }
...
function setupNoteProductPicker(select){
  ...
  const render=()=>{const query=search.value.trim().toLocaleLowerCase('hr'),options=[...select.options];list.innerHTML=options.filter((option)=>!query||`${option.textContent} ${option.dataset.search||''}`.toLocaleLowerCase('hr').includes(query)).map((option)=>{const data=getNoteOptionData(option);return`<button type="button" class="modern-multi-option ${option.selected?'selected':''}" data-picker-value="${escapeHTML(option.value)}" aria-pressed="${option.selected}"><img src="${escapeHTML(data.image)}" alt=""><span class="modern-multi-copy"><strong>${escapeHTML(data.name)}</strong><small>${escapeHTML(data.meta||'Bez dodatnih podataka')}</small></span><b aria-hidden="true">${option.selected?'✓':'+'}</b></button>`;}).join('')||'<p class="modern-multi-empty">Nema dresova za ovu pretragu.</p>';const selected=options.filter((option)=>option.selected);chips.innerHTML=selected.length?selected.map((option)=>{const data=getNoteOptionData(option);return`<button type="button" class="modern-multi-selected-card" data-remove-picker-value="${escapeHTML(option.value)}"><img src="${escapeHTML(data.image)}" alt=""><span class="modern-multi-copy"><strong>${escapeHTML(data.name)}</strong><small>${escapeHTML(data.meta||'Bez dodatnih podataka')}</small></span><b aria-hidden="true">×</b></button>`;}).join(''):'<span class="modern-multi-placeholder">Nema odabranih dresova</span>';};
