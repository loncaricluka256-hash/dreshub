import { getAllProducts, getProductById, getProductMainImage, saveProducts, duplicateSneakerProduct, duplicateJerseyProduct, setProductVersion } from '../services/productService.js';
import {
...
import { createTransaction, createLiveSale } from '../services/transactionService.js';
...
function setupOtherProductsAdminUI(){
  const dresList=document.querySelector('[data-products-list]'),addDuplicateButtons=()=>dresList?.querySelectorAll('[data-admin-product]').forEach((row)=>{const actions=row.querySelector('.row-actions'),product=products.find((item)=>String(item.id)===String(row.dataset.adminProduct));if(!actions)return;if(!actions.querySelector('[data-duplicate-product]'))actions.insertAdjacentHTML('afterbegin','<button data-duplicate-product title="Dupliciraj dres">⧉</button>');if(!actions.querySelector('[data-sell-product]'))actions.insertAdjacentHTML('afterbegin',`<button class="sell-product-button" data-sell-product title="Prodano" ${!product||product.stock<1?'disabled':''}>Prodano</button>`);});if(dresList)new MutationObserver(addDuplicateButtons).observe(dresList,{childList:true});
...
function renderFinance() {
  ...
  const all=getCollection('transactions'), sales=all.filter(i=>i.type==='Prodaja').reduce((s,i)=>s+Math.abs(i.amount),0), costs=all.filter(i=>negativeTypes.has(i.type)).reduce((s,i)=>s+Math.abs(i.amount),0), stockValue=products.filter(p=>!p.archived).reduce((s,p)=>s+Number(p.costPrice||0)*p.stock,0), sold=getReservations().filter(r=>r.status==='completed').reduce((s,r)=>s+r.items.reduce((x,i)=>x+i.quantity,0),0)+all.reduce((sum,item)=>sum+(item.items||[]).reduce((count,sale)=>count+Number(sale.quantity||1),0),0);
...
function bindProducts() {
  ...
  document.addEventListener('click',(event)=>{const button=event.target.closest('[data-sell-product]');if(!button)return;event.preventDefault();event.stopPropagation();const row=button.closest('[data-admin-product]'),product=products.find((item)=>String(item.id)===String(row?.dataset.adminProduct));openLiveSaleDialog(product);});
...
function openLinkedProductDetail(productId){const product=products.find((item)=>String(item.id)===String(productId));if(!product){toast('Povezani dres više nije dostupan.');return;}const notes=getCollection('notes').filter((note)=>(note.productIds||[note.productId]).some((id)=>String(id)===String(product.id))),transactions=getCollection('transactions').filter((entry)=>(entry.items||[]).some((item)=>String(item.productId)===String(product.id))),dialog=document.querySelector('[data-linked-product-dialog]');dialog.querySelector('[data-linked-product-detail]').innerHTML=`<header><div><p class="eyebrow">Detalji dresa</p><h2>${escapeHTML(product.name)}</h2></div><button type="button" data-close-dialog>×</button></header><div class="linked-product-gallery">${(product.images?.length?product.images:[getProductMainImage(product)]).map((image)=>`<img src="${escapeHTML(image)}" alt="${escapeHTML(product.name)}">`).join('')}</div><span class="status-pill ${product.archived?'cancelled':'completed'}">${product.archived?'Arhivirano / prodano':'Aktivan'}</span><dl class="detail-list"><div><dt>Klub / reprezentacija</dt><dd>${escapeHTML(product.club||'-')}</dd></div><div><dt>Igrač</dt><dd>${escapeHTML(product.player||'-')}</dd></div><div><dt>Veličina</dt><dd>${escapeHTML(product.sizes.join(', ')||'-')}</dd></div><div><dt>Verzija</dt><dd>${escapeHTML(product.version||'-')}</dd></div><div><dt>Kupovna cijena</dt><dd>${formatPrice(product.costPrice)}</dd></div><div><dt>Prodajna cijena</dt><dd>${formatPrice(product.price)}</dd></div><div><dt>Trenutno stanje</dt><dd>${product.stock}</dd></div></dl><section><h3>Bilješke</h3>${notes.length?notes.map((note)=>`<p>${escapeHTML(note.title)} — ${escapeHTML(note.description||'Bez opisa')}</p>`).join(''):'<p>Nema povezanih bilješki.</p>'}</section><section><h3>Transakcije</h3>${transactions.length?transactions.map((entry)=>`<p>${formatDate(entry.date)} — ${formatPrice(entry.items.find((item)=>String(item.productId)===String(product.id))?.price||0)}</p>`).join(''):'<p>Nema povezanih transakcija.</p>'}</section>`;dialog.showModal();}
