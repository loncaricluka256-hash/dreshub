...
export function getReservationById(reservationId) { return reservationCache.find((item) => item.id === Number(reservationId)) ?? null; }

/**
 * Stvara rezervaciju, smanjuje zalihe i sprema poslovni zapis.
 * @param {{productId?:number,items?:Array<Object>,name:string,phone:string,note?:string}} reservationData Podaci.
...
export async function createReservation(reservationData) {
  ...
  const requestedItems = reservationData.items?.length ? reservationData.items : [{ productId: Number(reservationData.productId), quantity: 1 }];
  const items = [];
  for (const requested of requestedItems) {
    const product = await getProductById(requested.productId), quantity = Math.max(1, Number(requested.quantity || 1));
  ...
  const client = await getSupabaseClient();
  if (client) {
    const item=items[0],payload = { product_id:item.productId,customer_name:record.name,customer_phone:record.phone,note:record.note,status:record.status,reserved_price:Number(item.price),expires_at:record.expiresAt,created_at:record.createdAt };
    ...
    const saved = fromDatabase(data,await getProductById(data.product_id)); reservationCache = [saved, ...reservationCache]; return saved;
  }
